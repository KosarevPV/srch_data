...
def main():
    ...
    jobs = pd.DataFrame(data_list, columns=['name', 'employer', 'location', 'min_salary',
                                            'max_salary', 'href', 'source'])
    jobs.to_excel(f'{datetime.date.today()}_jobs_{"_".join(post.split())}.xlsx', index=False)
    print(jobs)
